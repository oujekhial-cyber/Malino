...
## تشخیص کرش بدون کامپیوتر

- اگر برنامه روی گوشی کرش کند، هندلر کرش (`MalinoCrashReporter` که به‌صورت
  ContentProvider پیش از Activity ثبت می‌شود) گزارش کامل کرش را در
  `Download/malino_crash.txt` ذخیره می‌کند؛ فایل را از برنامه Files/Files
  باز کنید و متنش را بفرستید.
- در CI یک جاب `emulator-test` وجود دارد: هر بیلد به‌طور خودکار روی شبیه‌ساز
  اندروید ۱۴ (API 34) نصب و اجرا می‌شود؛ اگر برنامه بلافاصله بسته شود، جاب
  قرمز می‌شود و لاگ کرش (logcat) به‌عنوان کامیت روی همان کامیت ثبت می‌شود.
  برای همین تست یک APK دوم برای x86_64 هم ساخته می‌شود.
