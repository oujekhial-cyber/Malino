...
class ObligationsAndSmsHistoryGuardTest {
 @Test fun `database migration adds debts payments and checks`() { val s=File("src/main/java/ir/kharjyar/app/data/db/KharjYarDatabase.kt").readText();assertTrue(s.contains("version = 4"));assertTrue(s.contains("MIGRATION_3_4"));assertTrue(s.contains("debt_payments"));assertTrue(s.contains("checks")) }
