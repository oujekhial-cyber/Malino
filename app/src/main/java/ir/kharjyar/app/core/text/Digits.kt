...
object Digits {
    ...
    /**
     * نرمال‌سازی متن برای «تطبیق» (نه نمایش).
     *
     * پیامک بانک‌ها اغلب با حروف عربی نوشته می‌شود («خريد» با ي عربی، «كسر» با
     * ك عربی) و همین باعث می‌شد کلیدواژه‌های فارسی پیدا نشوند و هر پیامک دوباره
     * از کاربر پرسیده شود. اینجا ارقام، حروف هم‌شکل عربی، نیم‌فاصله، کشیدگی و
     * فاصله‌های تکراری یکدست می‌شوند.
     */
    fun normalizeForMatch(input: String): String {
        val sb = StringBuilder(input.length)
        for (ch in normalize(input)) {
            sb.append(
                when (ch) {
                    'ي', 'ى', 'ﯼ', 'ﯽ', 'ئ' -> 'ی'
                    'ك', 'ﻙ', 'ﻚ' -> 'ک'
        ...
        // فاصله‌های تکراری به یک فاصله؛ خط‌های جدید حفظ می‌شوند
