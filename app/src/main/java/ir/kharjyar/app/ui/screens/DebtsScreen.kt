...
@Composable fun DebtsScreen(vm: AppViewModel) {
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp), verticalArrangement=Arrangement.spacedBy(12.dp)) {
        ...
        OutlinedTextField(person,{person=it},label={Text("نام شخص")},modifier=Modifier.fillMaxWidth()); OutlinedTextField(title,{title=it},label={Text("عنوان/توضیح")},modifier=Modifier.fillMaxWidth()); OutlinedTextField(amount,{amount=it},label={Text("مبلغ به ریال")},modifier=Modifier.fillMaxWidth())
        ...
        Button({ scope.launch { val p=people.firstOrNull{it.name==person}; val pid=p?.id?:vm.repo.db.debtDao().insertPerson(DebtPersonEntity(name=person,createdAt=System.currentTimeMillis())); vm.repo.db.debtDao().insertDebt(DebtEntity(personId=pid,kind=kind,amountRial=Digits.parseAmount(amount)?:0,title=title,createdAt=System.currentTimeMillis(),dueAt=due.startOfDayMillis(),reminderAt=due.startOfDayMillis())); person="";title="";amount="" } }, enabled=person.isNotBlank()&&(Digits.parseAmount(amount)?:0)>0, modifier=Modifier.fillMaxWidth()){Text("ثبت طلب/بدهی و یادآور")}
        HorizontalDivider()
        debts.forEach { debt -> val paid=payments.filter{it.debtId==debt.id}.sumOf{it.amountRial}; val remain=(debt.amountRial-paid).coerceAtLeast(0); val p=people.firstOrNull{it.id==debt.personId}
            Card(Modifier.fillMaxWidth()){Column(Modifier.padding(14.dp)){Text("${p?.name?:"؟"} — ${debt.title}",style=MaterialTheme.typography.titleMedium);Text((if(debt.kind==DebtKind.RECEIVABLE)"طلب: " else "بدهی: ")+Money.format(debt.amountRial,settings.moneyUnit));Text("مانده: ${Money.format(remain,settings.moneyUnit)}");Text("سررسید: ${debt.dueAt?.let{PersianDate.formatDateTime(it)}?:"—"}"); var pay by remember(debt.id){mutableStateOf("")};OutlinedTextField(pay,{pay=it},label={Text("ثبت پرداخت مرحله‌ای")});Button({scope.launch{val v=Digits.parseAmount(pay)?:return@launch;vm.repo.db.debtDao().insertPayment(DebtPaymentEntity(debtId=debt.id,amountRial=v,paidAt=System.currentTimeMillis()));if(v>=remain)vm.repo.db.debtDao().updateDebt(debt.copy(settled=true))}},enabled=remain>0){Text("ثبت پرداخت")}}}
        }
    }
}
